...
export default function example() {
	...
	for(let i = 0; i < spherePositionArray.length; i++){
		randomPositionArray.push((Math.random() - .5) * 10);
	};
	...
	for(let i = 0; i < spherePositionArray.length; i += 3){
		imagePanel = new ImagePanel({
			...
			imageSrc: `./images/0${Math.ceil(Math.random() * 5)}.png`,
		...
		// 		map: textureLoader.load(`/images/0${Math.ceil(Math.random() * 5)}.jpg`)
		// 	})
		// );
		// planeMesh.position.set(spherePositionArray[i],spherePositionArray[i+1], spherePositionArray[i+2]);
